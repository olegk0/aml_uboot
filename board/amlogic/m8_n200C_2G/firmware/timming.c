...
static struct ddr_set __ddr_setting={
	...
	.t_ddr_clk_ctrl = ( 1  << 0 ) | //[B0]: DDR PCTL PCLK enable.	before access PCTL regsiter, we need enable PCTL PCLK 24 cycles early.
					    		    //after access PCTL regsiter, we can disable PCTL PCTLK 24 cycles later.
					  ( 1  << 1 ) | //[B1]: DDR PUB PCLK auto clock gating enable.
					  ( 0  << 2 ) | //[B2]: Disable DDR0 PUB PCLK clock.
					  ( 1  << 3 ) | //[B3]: DDR PCTL n_clk auto clock gating enable.
					  ( 0  << 4 ) | //[B4]: DDR PCTL n_clk disable.
					  ( 1  << 5 ) | //[B5]: DDR PUB n_clk auto clock gating enable.  (may not be work).
					  ( 0  << 6 ) | //[B6]: DDR PUB n_clk disable.
					  ( 0  << 7 ) | //[B7]: NOT used. 
					  ( 1  << 8 ) | //[B8]: DDR PHY ctl_clk enable.  
					  ( 0  << 9 ),  //[B9]: DDR PHY RF mode.  we can invert the clock to DDR PHY by setting this bit to 1.
...
//	          (1 << 3) |    //[B3]two_t_en: DDR 2T mode, default is disable
	...
	.t_pub_zq0pr = 0x19,/*Impedance Control Register*/
	.t_pub_dxccr = 4| (/*0b11000100*/0b11110111<<5)|(3<<19), /*The DATX8 Common Configuration Register is used to control features that affect all DATX8 macros.
These include on-die termination enables and power-down enables for SSTL I/Os for all SDRAM data,
data mask and data strobe signals.

2 MDLEN Reset = 0x1
Master Delay Line Enable: Enables, if set, all DATX8 master delay line calibration to perform subsequent
period measurements following the initial period measurements that are performed after reset or on when
calibration is manually triggered. These additional measurements are accumulated and filtered as long as
this bit remains high. This bit is ANDed with the MDLEN bit in the individual DATX8

5-8 DQSRES Reset = 0x4
...
20 UDQPDR Reset = 0x1
Unused DQ Power Down Receiver: Powers down, when set, the input receiver on the I/O for unused DQ
pins.
21 UDQIOM Reset = 0x0
Unused DQ I/O Mode: Selects SSTL mode (when set to 0) or CMOS mode (when set to 1) of the I/O for
unused DQ pins.

*/

	.t_pub_acbdlr0 = 0, //Local Calibrated Delay Line Register?
	
	.t_pub_dcr = 0b10001011, /*0x8b - 0b10001011  DRAM Configuration Register (DCR)

2-0 DDRMD DDR Mode: Set to 0x3 for DDR3 mode.
3   DDR8BNK DDR 8-Bank: Indicates if set that the SDRAM used has 8 banks.
6-4 PDQ Primary DQ (DDR3 Only): Specifies the DQ pin in a byte that is designated as a primary pin  for Multi-Purpose Register (MPR) reads. Valid values are 0 to 7 for DQ[0] to DQ[7], respectively. TI recommends that this field be always programmed to 000b.
7   MPRDQ Reset = 0x0
...
TI recommends that this bit be always programmed to 0.


*/
	.t_pub_mr={/*SDRAM definitions controlled by the Mode Register*/
		...
		#endif
//(1 << 11)| // Termination Data Strobe is applicable for x8 DRAMs only
	...
	.t_pub_dtpr={/*DRAM Timing Parameters Register*/
...
//	.t_mmc_ddr_ctrl= (0b1101 << 24) | //[B25,B24]:   ddr chanel selection. 
...
//					(0xff<<16) | //[B16]: bank page policy.	
					(1<<16) | //[B16]: bank page policy.	
//					(2<<14) |    //[B15,B14]: undefined
//					(CONFIG_DDR_BANK_SET<<13) |	  //[B13]: ddr1 address map bank mode  1 =  address
					(0<<13) |	  //[B13]: ddr1 address map bank mode  1 =  address switch between 4 banks. 0 = address switch between 2 banks.	 
					(0<<12) |	 //[B12]: ddr1 rank size.  0, 1, one rank.  2 : 2 ranks. 
//					(CONFIG_DDR1_ROW_SIZE<<10) |  //[B11,B10]: ddr1 row size.  2'b01 : A0~A12.
					(0<<10) |  //[B11,B10]: ddr1 row size.  2'b01 : A0~A12.	2'b10 : A0~A13.  2'b11 : A0~A14.  2'b00 : A0~A15. 
//					(CONFIG_DDR1_COL_SIZE<<8)	| //[B9,B8]: ddr1 col size.	2'b01 : A0~A8,	 
					(0<<8)	| //[B9,B8]: ddr1 col size.	2'b01 : A0~A8,	  2'b10 : A0~A9.  
//					(0<<6)  |    //[B7,B6]: undefined
//					(CONFIG_DDR_BANK_SET<<5)  |    //[B5]: ddr0 address map bank mode	1 = 
   					(1<<5)  |    //[B5]: ddr0 address map bank mode	1 =  address switch between 4 banks. 0 = address switch between 2 banks.	
					(0<<4)  |	 //[B4]: ddr0 rank size.  0, 1, one rank.  2 : 2 ranks.	 
//					(CONFIG_DDR0_ROW_SIZE<<2)  |   //[B3,B2]: ddr0 row size.	2'b01 : A0~A12.  
					(3<<2)  |   //[B3,B2]: ddr0 row size.	2'b01 : A0~A12.   2'b10 : A0~A13.  2'b11 : A0~A14.	2'b00 : A0~A15. 
//					(CONFIG_DDR0_COL_SIZE<<0), 	   //[B1,B0]: ddr0 col size.	2'b01 : A0~A8, 
					(2<<0), 	   //[B1,B0]: ddr0 col size.	2'b01 : A0~A8,  2'b10 : A0~A9.  
